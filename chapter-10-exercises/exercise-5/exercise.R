...
baby_names_2013 <- baby_names$year == 2013

# What was the most popular female name in 2013?
most_popular_female_name_2013 <- baby_names_2013[baby_names_2013$prop == max(baby_names_2013$prop), "name"]
...
most_popular_in_year <- function(year) {
  names_year <- baby_names[baby_names$year == year, ]
  most_popular <- names_year[baby_names_year$prop == max(baby_names_year$prop), "name"]
}
...
number_in_million <- function(name, year) {
  name_popularity <- names[baby_names$year == year & baby_names$name == name, "prop"]
...
number_in_million("Manbir", 1998) # I expect zero lol
